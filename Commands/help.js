...
.addField("Prefix(es)",Array.isArray(client.prefix)?client.prefix.map(x=>"`"+x+"`").join(", "):prefix)
...
if(!client.commands.has(args[1].toLowerCase())) return msg.channel.send(`Not Found... do \`${client.dprefix}help\` for a list of available commands.`);
...
.setTimestamp()
.setAuthor(msg.guild?msg.guild.me.displayName:client.user.username,client.user.displayAvatarURL())
.addField("Usage",x.syntax?(client.dprefix+x.syntax):"No usage specified.")
